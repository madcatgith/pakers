...
class phpbbClass
{
	...
	static public function getPostList($limit = 5)
	{

		$topics      = array();
		$users      = array();
		$topicsQuery = 'select * from phpbb_topics order by topic_last_post_time desc limit ' . intval($limit);
		$topicsQuery = DB::Query($topicsQuery);
		
		while ($row = DB::GetArray($topicsQuery))
			$topics[] = $row;
		...
		return $tpl->fetch(BASEPATH . '/lib/wmpPHPBB/post_list.tpl');
